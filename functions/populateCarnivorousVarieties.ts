...
Deno.serve(async (req) => {
  try {
    ...
    const allPlantTypes = await base44.asServiceRole.entities.PlantType.filter({ category: 'carnivorous' });
    
    const carnivorousDefaults = {
      'PT_VENUS_FLYTRAP': {
    ...
    // Create default varieties for each type
    for (const pt of allPlantTypes) {
      const defaults = carnivorousDefaults[pt.plant_type_code];
      if (!defaults) continue;

      // Create one default variety per type
      const varietyData = {
        plant_type_id: pt.id,
        plant_type_name: pt.common_name,
        variety_name: `${pt.common_name} (Standard)`,
        description: `Standard ${pt.common_name.toLowerCase()} variety. ${pt.description || ''}`,
        ...defaults
      };

      try {
        const created = await base44.asServiceRole.entities.Variety.create(varietyData);
        results.created.push({ plant_type: pt.common_name, variety_id: created.id });
      } catch (error) {
        console.error(`Error creating variety for ${pt.common_name}:`, error);
      }
    }
