...
import { setGlobalUnits } from '@/hooks/useUnits';
import { 
  Sprout, 
  User, 
  MapPin, 
...
// ─── Country configs — controls labels, placeholders, and zone system ───
const COUNTRIES = [
  { code: 'US', name: 'United States',  postalLabel: 'ZIP Code',            postalPlaceholder: '12345',     regionLabel: 'State',           regionPlaceholder: 'IL',             zoneSystem: 'usda',  defaultUnits: 'imperial' },
  { code: 'CA', name: 'Canada',         postalLabel: 'Postal Code',         postalPlaceholder: 'K1A 0B1',   regionLabel: 'Province',        regionPlaceholder: 'ON',             zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'GB', name: 'United Kingdom', postalLabel: 'Postcode',            postalPlaceholder: 'SW1A 1AA',  regionLabel: 'County/Region',   regionPlaceholder: 'London',         zoneSystem: 'rhs',   defaultUnits: 'metric'   },
  { code: 'IE', name: 'Ireland',        postalLabel: 'Eircode',             postalPlaceholder: 'D02 AF30',  regionLabel: 'County',          regionPlaceholder: 'Dublin',         zoneSystem: 'rhs',   defaultUnits: 'metric'   },
  { code: 'AU', name: 'Australia',      postalLabel: 'Postcode',            postalPlaceholder: '2000',      regionLabel: 'State',           regionPlaceholder: 'NSW',            zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'NZ', name: 'New Zealand',    postalLabel: 'Postcode',            postalPlaceholder: '6011',      regionLabel: 'Region',          regionPlaceholder: 'Wellington',     zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'DE', name: 'Germany',        postalLabel: 'Postleitzahl (PLZ)',  postalPlaceholder: '10115',     regionLabel: 'Bundesland',      regionPlaceholder: 'Berlin',         zoneSystem: 'rhs',   defaultUnits: 'metric'   },
  { code: 'FR', name: 'France',         postalLabel: 'Code Postal',         postalPlaceholder: '75001',     regionLabel: 'Région',          regionPlaceholder: 'Île-de-France',  zoneSystem: 'rhs',   defaultUnits: 'metric'   },
  { code: 'IT', name: 'Italy',          postalLabel: 'CAP',                 postalPlaceholder: '00100',     regionLabel: 'Regione',         regionPlaceholder: 'Lazio',          zoneSystem: 'rhs',   defaultUnits: 'metric'   },
  { code: 'ES', name: 'Spain',          postalLabel: 'Código Postal',       postalPlaceholder: '28001',     regionLabel: 'Comunidad',       regionPlaceholder: 'Madrid',         zoneSystem: 'rhs',   defaultUnits: 'metric'   },
  { code: 'NL', name: 'Netherlands',    postalLabel: 'Postcode',            postalPlaceholder: '1012 AB',   regionLabel: 'Provincie',       regionPlaceholder: 'Noord-Holland',  zoneSystem: 'rhs',   defaultUnits: 'metric'   },
  { code: 'ZA', name: 'South Africa',   postalLabel: 'Postal Code',         postalPlaceholder: '8001',      regionLabel: 'Province',        regionPlaceholder: 'Western Cape',   zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'JP', name: 'Japan',          postalLabel: 'Postal Code (〒)',    postalPlaceholder: '100-0001',  regionLabel: 'Prefecture',      regionPlaceholder: 'Tokyo',          zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'IN', name: 'India',          postalLabel: 'PIN Code',            postalPlaceholder: '110001',    regionLabel: 'State',           regionPlaceholder: 'Delhi',          zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'BR', name: 'Brazil',         postalLabel: 'CEP',                 postalPlaceholder: '01001-000', regionLabel: 'Estado',          regionPlaceholder: 'SP',             zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'MX', name: 'Mexico',         postalLabel: 'Código Postal',       postalPlaceholder: '06600',     regionLabel: 'Estado',          regionPlaceholder: 'CDMX',           zoneSystem: 'usda',  defaultUnits: 'metric'   },
  { code: 'OTHER', name: 'Other',       postalLabel: 'Postal/ZIP Code',     postalPlaceholder: '',          regionLabel: 'Region/Province', regionPlaceholder: '',               zoneSystem: 'usda',  defaultUnits: 'metric'   },
];
...
export default function Onboarding() {
  ...
  const isUSDA = countryConfig.zoneSystem === 'usda';
  ...
  useEffect(() => {
    try {
      ...
      if (browserCountry) {
        const matched = COUNTRIES.find(c => c.code === browserCountry);
        if (matched) {
          setFormData(prev => ({
            ...prev,
      ...
      // Keep defaults if locale detection fails
    }
  }, []);
  ...
  const handleCountryChange = (code) => {
    ...
    const prevConfig = countryConfig;
    setFormData(prev => ({
      ...prev,
      country: code,
      units: config.defaultUnits,
      // Clear zone if switching between zone systems (USDA ↔ RHS)
      usda_zone: config.zoneSystem !== prevConfig.zoneSystem ? '' : prev.usda_zone
    }));
  };

  const handleUnitsChange = (value) => {
    handleChange('units', value);
    // Immediately propagate to any mounted components via the global hook
  ...
  const handleComplete = async () => {
    ...
    setLoading(true);
    try {
      // ─── Save ALL profile fields including country + units ───
      ...
      // Propagate units globally so everything in the app picks it up immediately
      setGlobalUnits(formData.units);
      ...
      // Default plot dimensions — slightly different for metric users
      const plotW = formData.units === 'metric' ? 1200 : 480;   // ~10m or 40ft (in inches)
      const plotH = formData.units === 'metric' ? 1800 : 720;   // ~15m or 60ft (in inches)

      await base44.entities.GardenPlot.create({
        garden_id: garden.id,
        width: plotW,
        height: plotH,
        units: formData.units === 'metric' ? 'm' : 'ft',
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Used to set labels, default units, and zone system for your region
              </p>
            </div>
      ...
      case 'location':
        return (
          <div className="space-y-6">
            ...
            <div>
              <Label htmlFor="zip">{countryConfig.postalLabel}</Label>
              <Input
                id="zip"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="state">{countryConfig.regionLabel}</Label>
                <Input
                  id="state"
                  placeholder={countryConfig.regionPlaceholder}
            ...
            {/* Hardiness Zone — different UI for USDA vs RHS countries */}
            <div>
              <Label>
                {isUSDA ? 'USDA Hardiness Zone' : 'Hardiness Zone'}
              </Label>

              {isUSDA ? (
                /* ── USDA dropdown (US, Canada, Australia, etc.) ── */
                ...
                /* ── RHS + USDA combined dropdown (UK, Europe) ── */
                <Select value={formData.usda_zone} onValueChange={(v) => handleChange('usda_zone', v)}>
                  <SelectTrigger className="mt-2">
                    <SelectValue placeholder="Select your hardiness rating" />
                  </SelectTrigger>
                  <SelectContent className="max-h-72">
                    ...
                    {/* Divider + USDA section */}
                    <div className="px-2 py-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wider border-t mt-1 pt-2">
                      USDA Equivalent (if known)
                    </div>
                    {USDA_ZONES.map(zone => (
                      <SelectItem key={`usda-${zone}`} value={zone}>USDA Zone {zone}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              )}
      ...
      case 'frost':
        return (
          <div className="space-y-6">
            ...
            <div className="p-4 bg-purple-50 rounded-xl border border-purple-200">
              ...
              <p className="text-sm text-purple-800 mb-3">
                {formData.country === 'US'
                  ...
                  : `We'll use your ${countryConfig.postalLabel.toLowerCase()}, city, and country to estimate your hardiness zone and frost dates.`
                }
              </p>
              <FrostDateLookup
            ...
            <div className="p-4 bg-blue-50 rounded-xl border border-blue-200">
              ...
              <p className="text-sm text-blue-800 mb-3">
                {formData.country === 'US' 
                  ...
                  : `Search online for "frost dates ${countryConfig.name}" or "last frost [your city]" to find local data.`
                }
              </p>
              {formData.country === 'US' ? (
            ...
            <div>
              <Label htmlFor="last_frost">
                Last Spring Frost Date {formData.units === 'metric' ? '(0°C)' : '(32°F)'} *
              </Label>
              <Input
                id="last_frost"
                type="date"
            ...
            <div>
              <Label htmlFor="first_frost">
                First {formData.country === 'US' ? 'Fall' : 'Autumn'} Frost Date {formData.units === 'metric' ? '(0°C)' : '(32°F)'} *
              </Label>
              <Input
                id="first_frost"
                type="date"
      ...
      case 'garden':
        return (
          <div className="space-y-6">
            ...
            {/* Settings summary so user can verify before finishing */}
            <div className="p-4 bg-gray-50 rounded-xl border space-y-2">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm">
                ...
                <span className="font-medium text-gray-900">{countryConfig.name}</span>
                
                <span className="text-gray-500">Units:</span>
                <span className="font-medium text-gray-900">{formData.units === 'metric' ? 'Metric (m, cm, °C)' : 'Imperial (ft, in, °F)'}</span>
                
                {formData.usda_zone && (
                  <>
                    <span className="text-gray-500">Zone:</span>
                    <span className="font-medium text-gray-900">
                      {formData.usda_zone.startsWith('H') ? formData.usda_zone : `Zone ${formData.usda_zone}`}
                    </span>
                  </>
                )}
                {formData.last_frost_date && (
                  <>
                    ...
                    <span className="font-medium text-gray-900">
                      {new Date(formData.last_frost_date + 'T12:00:00').toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                    </span>
                  </>
                )}
                {formData.first_frost_date && (
                  <>
                    ...
                    <span className="font-medium text-gray-900">
                      {new Date(formData.first_frost_date + 'T12:00:00').toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                    </span>
                  </>
                )}
              </div>
